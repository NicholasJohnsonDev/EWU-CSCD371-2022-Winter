...
public class ProgramTests
{
    ...
    [TestMethod]
    public void RandomizeQuestions_QuestionInDifferentOrder()
    {
        // Arrange 
        string filePath = Program.GetFilePath();
        Question[] questions = Program.LoadQuestions(filePath);
        Question[] randomizedQuestions = Program.RandomizeQuestions(questions);
        bool mathces = true;

        // Act
        for (int i = 0; i < questions.Length; i++)
        {
            if (questions[i].Text != randomizedQuestions[i].Text)
            {
                mathces = false;
            }
        }
